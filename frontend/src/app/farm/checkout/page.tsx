    ...
    try {
      const res = await fetch('/api/farm/orders', {
        ...
        body: JSON.stringify({
          ...
          items: cart.map(i => ({ id: i.id, name: i.name, price: i.price, emoji: i.emoji, quantity: i.quantity })),
        }),
      });
  ...
  if (orderNumber) {
    return (
      <div className="min-h-screen bg-amber-50 flex flex-col items-center justify-center px-6 text-center" dir="rtl">
        ...
        <div className="bg-amber-700 text-amber-50 text-2xl font-bold px-6 py-3 rounded-2xl my-2 tracking-wider">
          #{orderNumber}
        </div>
        ...
        <button
          onClick={() => router.push('/farm/menu')}
          className="mt-6 text-amber-700 font-semibold text-sm underline underline-offset-2"
        >
          חזרה לתפריט
        </button>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-amber-100 shadow-sm overflow-hidden">
          <div className="px-4 py-3 bg-amber-50 border-b border-amber-100">
            <h2 className="font-bold text-amber-800">ההזמנה שלך</h2>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-amber-100 shadow-sm p-4">
          ...
          <input
            ...
            placeholder="למשל: מיכל, יוסי..."
            className="w-full border border-amber-200 rounded-xl px-4 py-3 text-stone-800 placeholder-stone-300 focus:outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-200 text-base"
            autoComplete="off"
          />
        </div>
        ...
        <div className="bg-white rounded-2xl border border-amber-100 shadow-sm p-4">
          ...
          <input
            ...
            placeholder="למשל: ליד האורווה, ללא סוכר..."
            className="w-full border border-amber-200 rounded-xl px-4 py-3 text-stone-800 placeholder-stone-300 focus:outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-200 text-base"
          />
        </div>
        ...
        <button
          ...
          className="w-full bg-amber-700 text-amber-50 font-bold py-4 rounded-2xl text-base shadow-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-amber-800 active:scale-95 transition-all"
        >
